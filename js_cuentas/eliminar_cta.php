...
//	$id=intval($_POST['id']);
	...
	$sql="SELECT count(com_cuenta) AS cuantos FROM ".$_SESSION[institucion]."sgcaf820 WHERE com_cuenta=:codigo GROUP BY com_cuenta";
	try
	{
	...
	if ($con->rowCount() < 1)
	{
		$sql="DELETE FROM ".$_SESSION[institucion]."sgcaf810 WHERE cue_codigo=:codigo";
		$con=$db_con->prepare($sql);
		$query_delete = $con->execute(array(':codigo' =>$codigo));
		if ($query_delete)
