...
session_start();
	include_once('../funciones.php');
	 if (empty($_POST['codigo'])){
		...
		$codigo = $_POST['codigo'];
		$nombre = $_POST['nombre'];
		$codigosudeca = $_POST['codigosudeca'];
		$nombresudeca= $_POST['nombresudeca'];
		include_once('../dbconfig.php');
		$sql="UPDATE ".$_SESSION['institucion']."sgcaf810 SET cue_nombre=:nombre, codsudeca=:codigosudeca, nomsudeca=:nombresudeca WHERE cue_codigo=:codigo";
		...
		try
		{
			$query_update = $con->execute(array(
				":codigo"=>$codigo,
				":nombre"=>$nombre,
				":codigosudeca"=>$codigosudeca,
				":nombresudeca"=>$nombresudeca,
				));
		}
