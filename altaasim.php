...
if (!$asiento) 
{
  ...
  $res1->execute(array(":asiento"=>$asiento));
	...
	$result = $db_con->prepare("SELECT date_format(con_ultfec,'%d/%m/%y') AS ultfechax FROM ".$_SESSION['institucion']."sgcaf8co");
  $result->execute();
...
<?php
  ...
  $expli=$db_con->prepare("SELECT enc_explic FROM ".$_SESSION['institucion']."sgcaf830 WHERE enc_clave = '".$_POST['asiento']."'");
  $expli->execute();
