...
{
   // echo '<div id="div1">';
    $sql= "SELECT *, date_format(enc_fecha, '%d/%m/%Y') as fecha FROM ".$_SESSION['institucion']."sgcaf830 where enc_clave=:numero"; 
...
//	if ($mes==$mesn and $ano==$anon)
	try
	{
		$sql= "SELECT * FROM ".$_SESSION['institucion']."sgcaf820 where com_nrocom=:numero"; 
		$rs=$db_con->prepare($sql);
		$rs->execute([":numero"=>$numero]);
		// echo $sql;
		while ($row = $rs->fetch(PDO::FETCH_ASSOC))
		{
			$sql="UPDATE ".$_SESSION['institucion']."sgcaf820 SET com_fecha=:fechanueva WHERE com_nrocom =:numero";
// 		   	echo $sql;
		...
		$sql="UPDATE ".$_SESSION['institucion']."sgcaf830 SET enc_fecha=:fechanueva WHERE enc_clave =:numero";
    //	echo $sql;
		$res=$db_con->prepare($sql);
		// echo $sql;
		$res->execute([":fechanueva"=>$fechanueva,":numero"=>$numero]);
