...
//echo $asiento.'zzz';
// echo 'n'.$_GET['n'] ;
//$sql="SELECT enc_clave FROM ".$_SESSION['institucion']."sgcaf830 WHERE enc_clave = '$asiento'")) AND $_GET['n'] == 1;
...
$sql="SELECT date_format(con_lpini,'%y') AS anocont FROM ".$_SESSION['institucion']."sgcaf8co";
...
//	$a=explode("/",$fecha); 
	...
	if ((reviso($_POST['cuenta1'], $db_con)) AND (reviso($_POST['cuenta2'], $db_con))) {
		...
		if ($res->rowCount() < 0)
		{
			$sql = "INSERT INTO ".$_SESSION['institucion']."sgcaf830 (enc_clave, enc_fecha, enc_desco, enc_desc1, enc_debe, enc_haber, enc_item, enc_dif, enc_igual, enc_refer, enc_sw, enc_explic) VALUES ('$asiento', '$b', '','',0,0,0,0,0,0,0,\"$explicacion\")"; 
			$res=$db_con->prepare($sql);
			$res->execute();
			if (!$res) die ("El usuario $usuario no tiene permiso para añadir Asientos.");
		}
		if ($nomatach) {
			$sql="UPDATE ".$_SESSION['institucion']."sgcaf830 SET enc_explic = \"$explicacion\" WHERE enc_clave = '$asiento'";
			$res=$db_con->prepare($sql);
			$res->execute();
		}
