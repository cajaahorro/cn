...
*/
		echo '</div>';
	echo '</div>';
	$sql="SELECT con_nivel FROM ".$_SESSION['institucion']."sgcafniv order by con_nivel";
	...
	//		print_r($losniveles);
		...
		$proceados+=chequear_procesar($diciembre,$losniveles,12, $db_con, $seleccion, $proceados);
			echo "<script>callprogress(".round(100).")</script>"; //llamo a la función JS(JavaScript) para actualizar el progreso
			flush(); 
			ob_flush();
	}
