...
if ($accion == "boasi") {
 	$sql = "DELETE FROM ".$_SESSION['institucion']."sgcaf820 WHERE com_nrocom = :asiento";
 	$res=$db_con->prepare($sql);
 	$res=$res->execute(array(":asiento"=>$asiento));
	if (!$res) die ("El usuario $usuario no tiene permiso para borrar Asientos.");
	$sql = "DELETE FROM ".$_SESSION['institucion']."sgcaf830 WHERE enc_clave = :asiento";
 	$res=$db_con->prepare($sql);
 	$res=$res->execute(array(":asiento"=>$asiento));
	...
	$sql_bita="insert into ".$_SESSION['institucion']."sgcabita (cuento, ip, quien) values ('$cuento', '$ip', '$usuario')";
...
if ($asiento) {
	$result = "SELECT enc_clave, enc_explic FROM ".$_SESSION['institucion']."sgcaf830 WHERE enc_clave = '$asiento'";
	$res=$db_con->prepare($result);
	$res->execute();
	$con_830=$res->rowCount();
	$result = "SELECT com_nrocom FROM ".$_SESSION['institucion']."sgcaf820 WHERE com_nrocom = '$asiento'";
	$res=$db_con->prepare($result);
	$res->execute();
