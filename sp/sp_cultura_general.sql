...
CREATE PROCEDURE sp_blc_comprobacion (IN xml_input MEDIUMTEXT, IN campos INT )
...
BEGIN
  ...
  -- SET @variable:='cue_cre01+cue_cre02';
--  SET @variable := campos;
  SET row_count=3;
  SET xpath = "";
  WHILE k < campos DO
   SET k := k + 1;
   SET xpath = concat(xpath, 'cue_cre0', k);
   IF (k < campos) THEN
   SET xpath = concat(xpath, '+');
   END IF ;
  END WHILE;
  -- SET xpath = concat(xpath, 'cue_cre04');
  SET @variable= xpath;
-- select xpath;
--  SET xpath = concat(xpath, 'cue_cre04  AS debe');
  SET  @sql=concat('SELECT cue_codigo, cue_nombre, ',@variable,' as debe FROM CAPPOUCLA_sgcaf810 WHERE substr(cue_codigo,1,1)="1" and cue_nivel="7" ORDER BY cue_codigo LIMIT 15'); 
...
CALL sp_blc_comprobacion('nada');
call sp_blc_comprobacion('nada',1);
