  DELIMITER $$
  DROP PROCEDURE IF EXISTS `sp_blc_comprobacion` $$
  CREATE PROCEDURE sp_blc_comprobacion (IN campos INT, IN todos INT, IN cuenta TEXT)

  BEGIN
    DECLARE k INT UNSIGNED DEFAULT 0;
    DECLARE debe, haber TEXT;
    DECLARE debes, habers TEXT;
    DECLARE ssql TEXT;
    -- meses anteriores
    SET debe = "";
    SET haber = "";
    ...
    WHILE k < campos DO
     SET k := k + 1;
     SET debe = concat(debe, 'cue_deb');
     SET haber = concat(haber, 'cue_cre');
     IF (k < 10) THEN
       SET debe = concat(debe, '0');
       SET haber = concat(haber, '0');
     END IF;
     SET debe = concat(debe, k);
     SET haber = concat(haber, k);
     IF (k < campos) THEN
       SET debe = concat(debe, '+');
       SET haber = concat(haber, '+');
     END IF ;
    END WHILE;
    SET @debe= debe;
    SET @haber= haber;
    ...
    SET debes ="";
    SET habers ="";
    SET debes = concat(debes, 'cue_deb');
    SET habers = concat(habers, 'cue_cre');
    IF (campos < 10) THEN
      SET debes= concat(debes, '0');
      SET habers = concat(habers, '0');
    END IF;
    SET @debes = concat(debes, campos);
    SET @habers = concat(habers, campos);

    SET @ssql=concat('SELECT cue_codigo, cue_nombre, cue_nivel, cue_saldo, ',@debe,' as danterior, ',@haber,' as hanterior');
    -- select debes;
    SET @ssql=concat(@ssql,', ', @debes,' as debe');
    SET @ssql=concat(@ssql,', ', @habers,' as haber ');
    SET @ssql=concat(@ssql,' FROM CAPPOUCLA_sgcaf810');
    IF  (todos = 0) THEN
      SET @cuenta = cuenta;
      SET @ssql=concat(@ssql,' WHERE cue_codigo = "',@cuenta, '"'); 
    END IF;
    SET @ssql=concat(@ssql,' ORDER BY cue_codigo, cue_nivel'); 
    -- select @ssql;
...
CALL sp_blc_comprobacion(2, 0, '1-01-01');
